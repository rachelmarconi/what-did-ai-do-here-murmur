...
<script>
  ...
  /**
   * The position of the axis and tick marks.
   * @type {"top" | "bottom"} [position="bottom"]
   */
...
<g class="axis x-axis" class:snapTicks>
  {#each tickVals as tick, i (tick)}
    <g class="tick tick-{i}" transform="translate({$xScale(tick)},{position === "bottom" ? Math.max(...$yRange) : 0})">
      {#if gridlines !== false}
        <line class="gridline" y1={position === "bottom" ? ($height * -1) : $height } y2="0" x1="0" x2="0" />
      {/if}
      {#if tickMarks === true}
        <line
          class="tick-mark"
          y1={0}
          y2={6 * (position === "bottom" ? 1 : -1)}
